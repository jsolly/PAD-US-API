...
class AOI_IntersectViewTests(SetUp):
    def setUp(self):
        ...
        with open(
            os.path.join(self.base_dir, "AOI_Unprotected.json")
        ) as f:  # Middle of the Atlantic Ocean
    ...
    def test_no_aoi(self):
        ...
        self.assertJSONEqual(
            ...
            {"error": "aoi parameter is required"},
        )

    def test_get_intersected_features_for_single_aoi(self):
        url = reverse("aoi-intersect")
        data = {"aoi": self.AOI_Yosemite}
        response = self.client.post(url, data=json.dumps(data), content_type='application/json')

        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            len(response.json()), 2
        )  # There should be two features that overlap the AOI

    def test_get_no_features_for_unprotected_aoi(self):
        url = reverse("aoi-intersect")
        data = {"aoi": self.AOI_Unprotected}
        response = self.client.post(url, data=json.dumps(data), content_type='application/json')
    ...
    def test_get_overlap_percentage_for_single_aoi(self):
        ...
        data = {"aoi": self.AOI_Yosemite, "intersected_features": intersected_features}
        response = self.client.post(url, data=json.dumps(data), content_type='application/json')
